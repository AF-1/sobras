...
function _getRating(self)
	...
	local rating = "0"
	local loopRating, currentRatingInfo, loopComment, currentCommentInfo, loopLyrics, currentLyrics
	if server then
		server:userRequest(function(chunk,err)
				if err then
					...
					for i = 3, 5 do
						...
						if loopComment then
							...
							--log:info("comment = "..currentCommentInfo)
			...
			self.player:getId(),
			{ 'songinfo', 0 , 100, 'track_id:'..thisTrackID, 'tags:Rkw' }
		)
	end
end
