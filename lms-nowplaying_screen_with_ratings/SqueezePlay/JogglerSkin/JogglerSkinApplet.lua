...
function param(self)
	...
	return {
		...
		nowPlayingScreenStyles = {
			...
			{
				style = 'nowplaying_text_lyrics',
				artworkSize = midArtwork,
				text = self:string("TEXT_LYRICS"),
			},
			{
	...
	s.nowplaying = _uses(s.window, {
		...
		npprogress = {
			...
			npprogressB = {
				...
				w = screenWidth - _tracklayout.x - 2*60 - 25,
				h = 50,
				padding = { 0, 0, 0, 0 },
			        position = LAYOUT_SOUTH,
				horizontal = 1,
...
-------------------------------
-------------------------------
	local _lyricsProgressBar = _loadHTile(self, {
			imgpath .. "NowPlaying/sslyrics_progress_fill_l.png",
			imgpath .. "NowPlaying/sslyrics_progress_fill.png",
			imgpath .. "NowPlaying/sslyrics_progress_fill_r.png",
	})
	local _lyricsProgressBarBG = _loadHTile(self, {
			imgpath .. "NowPlaying/sslyrics_progress_fill_l.png",
			imgpath .. "NowPlaying/sslyrics_progress_fill_bg.png",
			imgpath .. "NowPlaying/sslyrics_progress_fill_r.png",
	})
	local _lyricsProgressBarPill = _loadImageTile(self, imgpath .. "NowPlaying/sslyrics_progress_slider.png")

	s.nowplaying_text_lyrics = _uses(s.nowplaying, {
		nptitle = {
			zOrder = 2,
			position = LAYOUT_NONE,
			x = 80,
		...
		nplyricsgroup = {
			...
			x          = 40,
			y          = TITLE_HEIGHT + 15,
			h          = screenHeight - TITLE_HEIGHT - 70 - 40,
			w          = screenWidth - 30,
			nplyrics = {
				w = WH_FILL,
				h = WH_FILL,
		...
		npprogress = {
			...
			npprogressB = {
				w = screenWidth - 120 - 130,
				h = 29,
				padding = { 0, 0, 0, 0 },
				position = LAYOUT_SOUTH,
				horizontal = 1,
				bgImg = _lyricsProgressBarBG,
				img = _lyricsProgressBar,
				pillImg = _lyricsProgressBarPill,
			},
		},
		npprogressNB = {
			zOrder = 2,
	...
	s.nowplaying_text_lyrics.pressed = _uses(s.nowplaying_text_lyrics)
	s.nowplaying_text_lyrics.nptitle.pressed = _uses(s.nowplaying_text_lyrics.nptitle)
	s.nowplaying_text_lyrics.nplyricsgroup.pressed = _uses(s.nowplaying_text_lyrics.nplyricsgroup)
-------------------------------
-------------------------------
-------------------------------
